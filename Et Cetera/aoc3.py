# functions!
# 1. Manhattan Distance (p1: Tuple, p2: Tuple) -> int
# 2. get route of path (p1: Tuple, direction: str) -> List[Tuple]
# trial and error to see which manhattno distance is closest
...
def movements(start: Tuple, directions: str) -> List[Tuple]:
...
def find_tangles(w1: List[Tuple], w2: List[Tuple]) -> List[Tuple]:
    tangles = []
...
def find_closest_manhattan_distance(origin: Tuple, tangles: List[Tuple]) -> Tuple:
    closest_point = tangles[0]

    man_dis = abs(closest_point[0] - origin[0]) + abs(closest_point[1] - origin[1])

    for i in range(len(tangles)):
        man_dis_temp = abs(tangles[i][0] - origin[0]) + abs(tangles[i][1] - origin[1])

        if man_dis_temp < man_dis:
            man_dis = man_dis_temp
            closest_point = tangles[i]
